...
export async function searchSimilarVectors(
  ...
  try {
    ...
    // Build the complete query using $queryRawUnsafe for vector operations
    // Prisma.sql doesn't handle vector type casting well, so we use unsafe with proper escaping
    ...
    if (microserviceIds && microserviceIds.length > 0) {
      ...
      whereClause += ` AND microservice_id IN (${placeholders})`;
      params.push(...microserviceIds);
      paramIndex += microserviceIds.length;
    }

    // Use $queryRawUnsafe with properly formatted vector literal
    // Escape single quotes in embedding string if any (shouldn't happen with numbers)
    ...
    if (results.length > 0) {
      ...
      const escapedEmbeddingStrTest = embeddingStr.replace(/'/g, "''");
      const vectorLiteralTest = `'${escapedEmbeddingStrTest}'::vector`;
      const testQuery = `
        SELECT 
          content_id,
          content_type,
          1 - (embedding <=> ${vectorLiteralTest}) as similarity
        FROM vector_embeddings
        WHERE tenant_id = $1
        ORDER BY embedding <=> ${vectorLiteralTest}
        LIMIT 10
      `;
